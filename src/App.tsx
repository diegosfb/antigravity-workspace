    ...
    <div className="h-[100dvh] overflow-hidden bg-neutral-950 text-white p-2 lg:p-4 font-sans relative flex flex-col">
      ...
      {inRoom && isMobile && (
        <button
          ...
          className="fixed top-4 left-4 z-[60] p-3 bg-neutral-900/90 backdrop-blur-md rounded-2xl border border-neutral-800 shadow-xl text-orange-500 active:scale-90 transition-transform"
        >
          {isSidebarOpen ? <Plus className="w-6 h-6 rotate-45" /> : <Plus className="w-6 h-6" />}
        </button>
      )}
        ...
        <AnimatePresence>
          {(isSidebarOpen || !isMobile) && (
            <motion.div
              ...
              className={`${isMobile ? 'fixed inset-y-0 left-0 w-72 z-50 p-6 pt-20 overflow-y-auto' : 'lg:col-span-3 h-full pr-1 overflow-y-auto scrollbar-hide'} space-y-4 bg-neutral-950/95 lg:bg-transparent backdrop-blur-xl lg:backdrop-blur-none border-r border-neutral-800 lg:border-none`}
            >
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center justify-between p-3 bg-neutral-800 rounded-xl border border-neutral-700">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold uppercase tracking-widest text-[9px]">BG Music</span>
                        </div>
                        <button
                          onClick={() => updateSettings({ backgroundMusicEnabled: !gameState.backgroundMusicEnabled })}
                          className={`w-8 h-4 rounded-full transition-colors relative ${gameState.backgroundMusicEnabled ? 'bg-purple-500' : 'bg-neutral-600'}`}
                        >
                          <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${gameState.backgroundMusicEnabled ? 'left-4.5' : 'left-0.5'}`} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 bg-neutral-800 rounded-xl border border-neutral-700">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold uppercase tracking-widest text-[9px]">WW2 Mode</span>
                        </div>
                        <button
                          onClick={() => updateSettings({ ww2Mode: !gameState.ww2Mode })}
                          className={`w-8 h-4 rounded-full transition-colors relative ${gameState.ww2Mode ? 'bg-orange-500' : 'bg-neutral-600'}`}
                        >
                          <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${gameState.ww2Mode ? 'left-4.5' : 'left-0.5'}`} />
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              <div key={opp.id} className={`bg-neutral-900/80 backdrop-blur-md p-2 rounded-2xl border border-neutral-800 relative overflow-hidden flex-shrink-0 ${isMobile ? 'w-24 h-32' : 'p-4 rounded-3xl'}`}>
                ...
                {opp.isGameOver && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                    <Ghost className={`${isMobile ? 'w-4 h-4' : 'w-8 h-8'} text-neutral-400`} />
                  </div>
                )}
              </div>
            ))}
            {opponents.length === 0 && (
              <div className={`${isMobile ? 'flex items-center justify-center w-full text-[10px]' : 'text-neutral-600 italic text-sm text-center py-12 border-2 border-dashed border-neutral-800 rounded-3xl'}`}>
                No opponents yet
              </div>
            )}
          </div>
